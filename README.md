...
A browser-based AR hide-and-seek proof of concept. A hider picks a pose, places
and paints a silhouette against a marker image, shares the hunt link, and a
seeker gets a limited number of taps to find it. MindAR tracks the marker,
...
The Phase 0 spike pages live in `tools/spikes/` and are no longer served — open
them as local files if you need to re-run those experiments.
